...
3. Aloud reads it back in a natural voice, streamed from
   [Kokoro](https://huggingface.co/hexgrad/Kokoro-82M) running locally via
   [MLX](https://github.com/ml-explore/mlx-swift), Apple's on-device ML framework. No text ever
   leaves your Mac.

There's no dock icon and no main window, just a menu bar icon that pulses while it's working,
and a small popover for playback controls, voice picking, and settings.
...
3. **First launch:** Aloud isn't notarized by Apple (no paid developer account behind this), so
   Gatekeeper will block it the first time. Right-click (or Control-click) `Aloud.app` in
...
4. Aloud walks you through the rest on first launch: granting Accessibility access (needed to
   read your text selection from other apps) and downloading the voice model (~340MB, one-time).
...
Click the menu bar icon to open the popover. It shows the current voice, playback controls, and
a caption of what's being read. Click the gear icon for Settings: pick a default voice (28
available, US/UK, male/female), change the hotkey, adjust reading speed, or turn on
launch-at-login.
...
- **Text capture**: reads the current selection via the Accessibility API. Falls back to a
  ...
  don't).
- **Synthesis**: [`mlalma/kokoro-ios`](https://github.com/mlalma/kokoro-ios), a from-scratch MLX
  Swift port of Kokoro-82M. Long selections are chunked into sentence-sized pieces and streamed
  to playback as they synthesize, rather than waiting for the whole thing.
...
Nothing about what you select or hear ever leaves your Mac. The only network traffic Aloud ever
makes is the one-time voice model download on first launch, from a pinned, checksum-verified
GitHub release.
...
Build and run from Xcode (⌘R). Swift Package Manager will resolve Kokoro, MLX, and the other
dependencies on first build. This can take a few minutes.
...
- [ml-explore/mlx-swift](https://github.com/ml-explore/mlx-swift): Apple's on-device ML
  framework
